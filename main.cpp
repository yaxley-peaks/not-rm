/*
This file is merely proof of concept and not feature complete. Do not use this
to do any serious things. Use the rust version instead. This file is not
maintained.
*/
...
std::unique_ptr<CLI> handle_args(char** args, int n) {
    ...
    switch (n) {
        case 1:
        case 2:
            std::cerr << "error: The following required arguments were not "
                         "provided:\n"
                         "    <INPUT>\n"
                         "    <EXT>\n"
                         "\n"
...
int main(int argc, char** argv) {
    ...
    for (const std::string& notPath : notPaths) {
        ...
        if (!fs::remove(path_rm))
            std::cout << "Could not remove: " << path_rm << '\n';
    }
    return 0;
}
